...
and generated into **Swift, Rust, TypeScript, Python, and Mojo** — with a
...
gen/                 generated by `dagr build` (git-ignored): gen/{swift,rust,typescript,python}/
examples/
...
- `cargo` / `rustc`, `swiftc` (macOS), `node` (v22+; the TS demo runs via `npx tsx`), `python3`,
  and `pixi` (for Mojo — the example builds against a Mojo pixi environment).

## Generate the code

```bash
dagr build        # reads schema.py, emits gen/{swift,rust,typescript,python,mojo}/, writes dagr.lock.json
...
== [byte-identity] all three minted tokens must be identical ==
  IDENTICAL (196 bytes) — rust == swift == ts == python == mojo
...
| Mojo | ✅ | ✅ | ✅ |
| Odin | ❌ — early-increment graph serde; no header | ❌ | — |
| Kotlin / Zig | ❌ — no header | — | — |

Two of these needed generator work to join:
...
**Odin** is still out: its graph serde is at an early increment that doesn't support
unions, frozen+packed nodes, or the header, and its graph emission isn't wired into
`dagr build` — a from-scratch serde build for this feature set. That's a generator
gap, not a format limit.
